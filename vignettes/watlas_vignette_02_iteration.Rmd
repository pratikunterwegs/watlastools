...
data_files <- list.files("vignette_data/",
  pattern = "413",
  full.names = TRUE
)
...
patch_data <- lapply(data, function(df) {

  # infer residence
  to_return <- wat_infer_residence(df)

  # classify points
  to_return <- wat_classify_points(to_return)

  # make patches
  to_return <- wat_make_res_patch(to_return)
...
patch_data <- patch_data[!vapply(patch_data, is.null, FUN.VALUE = T)]
sum(
  vapply(patch_data, nrow, FUN.VALUE = 1L)
)
...
From an initial unrepaired number of 157, we now have 153 patches.
